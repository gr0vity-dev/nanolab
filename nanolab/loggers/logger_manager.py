...
class LoggingManager:
    ...
    def __init__(self, logger_params, sink_params):
        ...
        self.sink_type = sink_params.pop("type", "console")

        self.logger_params = logger_params
        self.sink_params = sink_params

    async def create_logger_and_storage(self, current_logger_params):
        logger = LoggerFactory.create_logger(self.logger_type,
                                             current_logger_params)
        storage = SinkFactory.create_storage(self.sink_type, self.sink_params)
    ...
    async def create_loggers(self):
        for node in self.nodes_config:
            ...
            yield await self.create_logger_and_storage(current_params)

    async def start_logging(self):
        tasks = []
        async for logger, storage in self.create_loggers():
            tasks.append(
